...
;       USE_ALL  = Use full range of array, regardless of the number of points.
...
;       Version 7, 06-Jul-2007, William Thompson, GSFC
;               Increase number of points considered.  Added /USE_ALL keyword
;       Version 8, 10-Jul-2007, WTT, Call WHERE_NOT_MISSING
;-
;
...
;
        IF (N_ELEMENTS(ARRAY) LT 100000) OR KEYWORD_SET(USE_ALL) THEN $
          ATEMP0 = ARRAY ELSE BEGIN
            NN = 10000 > (N_ELEMENTS(ARRAY) / 5) < 1000000
            ATEMP0 = ARRAY[N_ELEMENTS(ARRAY)*RANDOMU(SEED,NN)]
        ENDELSE
;
;  Get the total range of the data, excluding any missing points.
;
        W = WHERE_NOT_MISSING(ATEMP0, MISSING=MISSING, COUNT)
        IF (COUNT GT 0) AND (COUNT LT N_ELEMENTS(ATEMP0)) THEN $
          ATEMP0 = ATEMP0(W)
