...
function vso::parse_daterange, range, tstart, tend, near, err=err
    ...
    if is_blank(tstart) then begin 
        if is_blank(near) then $
            message, 'No time range supplied (TSTART or named "date" or "start_date")'
        ; they specified 'near' with no tstart
        ; TODO : fix this so it gives a +/- 1hr range, rather than the sloppy logic
        ; that I'm using for now. (+/- 1day)
    ...
    dend = ''
    if is_blank(tend) and not is_struct(tend) then begin
...
function vso::buildQuery, tstart,tend,$
        ...
        spacecraft=spacecraft, observatory=observatory, $ ; synonyms for 'source'
        telescope=telescope, $ ; synonym for 'instrument'
        sample=sample, $ ; time sampling
    ...
    if ( num_sources eq 1 and not keyword_set( source ) ) then $
    ...
    if ( not keyword_set( instrument ) and keyword_set( telescope ) ) then $
...
function vso::getdata, args, methods=methods, merge=merge, urls=urls, _extra=extra
    ...
    dom = self->send('GetData', { request:{ VSOiGetDataRequest, version:version, request:{ GetDataRequest, data:data, method:methodPtr } } }, _extra=extra )
    ptr_free,methodPtr
    ptr_free,data
