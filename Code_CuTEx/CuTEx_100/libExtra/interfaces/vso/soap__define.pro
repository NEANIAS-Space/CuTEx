...
; helper objects                                                                                                                  
    ...
    if ( not obj_valid(self->getprop(/http))   ) then return, 0                                                                   
                                                                                                                                  
; make sure HTTP times out if something goes wrong                                                                                
    if since_version('6.0') then read_timeout=120 else read_timeout=0                                                             
    self.http->hset,read_timeout=read_timeout                                                                                     
                                                                                                                                  
;-- protect against pre-V 5.6                                                                                                     
                                                                                                                                  
    if since_version('5.6') then begin                                                                                            
        self.parser = obj_new('xmlparser', _extra=extra)                                                                          
        if ( not obj_valid(self->getprop(/parser)) ) then return, 0                                                               
...
;-- identify ourselves                                                                                                            
    ...
    self.http->hset,user_agent=user_agent                                                                                         
...
function soap::deserialize, xmlstring, method=method,_ref_extra=extra                                                             
    ...
    if ( not n_elements(method) ) then return, dom                                                                                
...
;    if (not n_elements(response)) then return, self->dom2struct(dom) ; need better error handling.                               
    if (not n_elements(response)) then return, dom ; need better error handling.                                                  
...
function soap::open, proxy, uri                                                                                                   
    ...
    if ( not stregex( proxy, '^http://', /boolean ) ) then begin                                                                  
...
; figures --  structures can't be cast as strings                                                                                 
    ...
    if ( not is_blank(name) ) then begin                                                                                          
...
;                                                                                                                                 
    ...
    switch ( size( item, /tname ) ) of                                                                                            
        ...
        'UNDEFINED':    return, '<' + name + ' xsi:null="1"/>'                                                                    
