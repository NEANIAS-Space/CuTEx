...
;-- fonts
    ...
    wbase=widget_mbase(title=wtitle,/column,group=group,modal=modal) 
...
;---------------------------------------------------------------------------
    ...
    if not (just_reg) then begin
     case_sense = 0
     if detach then $ 
      wbase1=widget_mbase(/column,event_pro='xtext_event',group=wbase,modal=modal) else $
       wbase1=widget_mbase(wbase,/column,modal=modal)
...
;-- cleanup
   ...
   if not just_reg then begin
    if idl_release(lower=5,/inc) then $
     xmanager, 'xtext', wbase,just_reg=just_reg,no_block=no_block,cleanup='xtext_cleanup' else $
      xmanager, 'xtext', wbase,just_reg=just_reg,cleanup='xtext_cleanup'
endif
