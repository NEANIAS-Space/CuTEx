...
if (bname eq 'remove') or (bname eq 'clear') then begin
 ...
 if delete_all then begin
  value=xanswer('Are you sure?',group=event.top)
  if value then begin 
   fields='' & nfields=0
...
;-- fonts
 ...
 wbase=widget_mbase(title=title,/column,group=group,modal=modal)
...
if not just_reg then xmanager,'xlist',wbase,just_reg=just_reg
