...
;-- check for overriding keywords
   ...
   if (1-vstruct_in) then begin
    ...
    need_rad=(1-is_number(r0)) and (1-is_number(rad))
    if need_rad or need_b0 then angles =pb0r(date,_extra=extra)
   endif
...
;---------------------------------------------------------------------------
   rotmx = [[COS(b0_r), 0.0, SIN(b0_r)], [0.0, 1.0, 0.0], [-SIN(b0_r), 0.0, COS(b0_r)]]
...
if (1-keyword_set(sphere)) then begin
...
;---------------------------------------------------------------------------
   ...
   if is_number(l0) then begin
    if l0 ne 0 then begin
     RETURN, !radeg*TRANSPOSE([[temporary(latitude)], $
     [(((5*!pi) + temporary(longitude)+((l0 mod 360.)/!radeg)) mod (2*!pi))-!pi]])
    endif
   endif
