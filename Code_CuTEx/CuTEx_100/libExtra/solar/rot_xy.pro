...
;-- validate time inputs
  ...
  if valid_time(date) and (1-valid_time(tstart)) then tstart=date
  if valid_time(tstart) and is_number(interval) then begin
   tend=anytim2utc(anytim2tai(tstart)+float(interval))
  endif else begin
   if valid_time(tstart) and valid_time(tend) then $
    interval=anytim2tai(tend)-anytim2tai(tstart)
  endelse

  if (1-is_number(interval)) or (1-valid_time(tstart)) or (1-valid_time(tend)) then begin
...
;---------------------------------------------------------------------------
    ...
    temp = TRANSPOSE(arcmin2hel(xx/60.0,yy/60.0,date=tstart,off=off,$
...
;---------------------------------------------------------------------------
   ...
   IF (interval EQ 0.d0) and (1-ang_in) THEN BEGIN
...
;-- differentially rotate if interval is non-zero
   ...
   IF KEYWORD_SET(keep) then tmp=tstart else tmp=tend
   back_index=-1
   IF offlimb EQ 0 THEN BEGIN
    temp = 60.0*TRANSPOSE(hel2arcmin([temp[*, 0]], [temp[*, 1]], $
           front,date=tmp,vstruct=vend,_extra=extra))
   ...
   ENDIF ELSE BEGIN
    IF in[0] GE 0 THEN BEGIN
     ...
     subtemp = 60.0*TRANSPOSE(hel2arcmin([subtemp[*, 0]], [subtemp[*, 1]],$
               subfront, _extra=extra,date=tmp,vstruct=vend))
     temp[in, *] = subtemp
     temp[out, *] = -9999.0
