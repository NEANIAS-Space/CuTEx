...
FUNCTION  pb0r, date, soho=soho, arcsec=arcsec, error=error, $
               retain=retain,earth=earth,debug=debug


   common pb0r_common, prev_output, prev_soho, prev_utc
...
;---------------------------------------------------------------------------
  ...
  utc = anytim2utc(date, /ecs, err=error)
  if error ne '' then get_utc, utc, /ecs
...
;---------------------------------------------------------------------------
   ...
   if use_soho then begin
    ...
    if have_orbit then begin
     ...
     pos = call_function('get_orbit',utc, errmsg=error, retain=ret_flag)
    endif else error='"get_orbit" function not in !path'

    if error ne '' then begin
     dprint,'% PB0R: '+error+', Earth view used with 1% correction'
     soho_sd = sd*1.01
     output = [p, b, soho_sd]
    endif else begin
     xe = pos.sun_vector_x
