...
self.dim1_sel=0b
if not self->have_plot_spectrogram() then begin
...
if is_string(extra) and not keyword_set( no_defaults ) then self->set_plot,_extra=extra
...
if exist( xrange ) and not exist( timerange ) then $
...
if not exist(plot_type) then begin
...
if not exist( xstyle ) then xstyle = self->get( /xstyle )
if not exist( ystyle ) then ystyle = self->get( /ystyle )
if not exist( ytitle ) then ytitle = self->get( /ytitle )

if plot_type_local ne 'specplot' or integrate ne 0 then begin

    ny = self->get(/ny)
    if not exist( dim1_use ) then begin
        this_dim1_use =  self->getprop( /dim1_use )
    endif else begin
        this_dim1_use = dim1_use
    endelse
    if not exist( dim1_fact ) then begin
    ...
    IF integrate ne 0 THEN BEGIN
        ...
        this_dim1_use = 0
        title = title + ' Integrated over ' + yint_range
