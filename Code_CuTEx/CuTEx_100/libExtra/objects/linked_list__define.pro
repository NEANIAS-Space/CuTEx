...
FUNCTION LINKED_LIST::INIT, item,no_copy=no_copy
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;
; PRO LINKED_LIST::ADD, item, index, AFTER=after, BEFORE=before
;
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;
; PRO LINKED_LIST::DELETE, index, ALL=all
;
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;
; FUNCTION LINKED_LIST::GET_COUNT
;
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;
; FUNCTION LINKED_LIST::GET_ITEM, index
;
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;
; FUNCTION LINKED_LIST::GET_NODE, index
;
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;
; PRO LINKED_LIST::HELP, PRINT=print
;
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;
; PRO LINKED_LIST::MOVE_NODE, nodeIndex, location, BEFORE=before
;
...
PRO LINKED_LIST::MOVE_NODE, nodeIndex, location, Before=before,err=err
...
PRO LINKED_LIST::DELETE_NODE, index,err=err
...
IF NOT Ptr_Valid((*currentNode).previous) THEN BEGIN
    ...
    if ptr_valid((*nextNode).previous) then heap_free, (*nextNode).previous
...
if ptr_valid(currentNode) then heap_free, currentNode
...
PRO LINKED_LIST::DELETE_LAST_NODE
...
IF NOT Ptr_Valid((*currentNode).previous) THEN BEGIN
    ...
    self.tail = Ptr_New()
    self.count = 0
    if ptr_valid((*currentNode).next) then heap_free, (*currentNode).next
ENDIF ELSE BEGIN
    previousNode = (*currentNode).previous
    self.tail = previousNode
    if ptr_valid((*self.tail).next) then heap_free, (*self.tail).next
...
PRO LINKED_LIST::DELETE_NODES

; This method deletes all of the nodes.

WHILE Ptr_exist(self.head) DO BEGIN
     currentNode = *self.head
     if ptr_valid(currentNode.previous) then heap_free, currentNode.previous
     if ptr_valid(currentNode.item) then heap_free, currentNode.item
     heap_free,self.head        ;-- added DMZ (4 July '00)
...
PRO LINKED_LIST::ADD_AFTER, item, index, no_copy=no_copy,err=err
...
currentNode = Ptr_New( {LINKED_LIST_NODE} )
...
PRO LINKED_LIST::ADD_BEFORE, item, index,no_copy=no_copy,err=err
...
PRO LINKED_LIST::ADD_TO_END, item,no_copy=no_copy,err=err
...
PRO LINKED_LIST::ADD, item, index, Before=before, After=after, $
...
function linked_list::get_value,index,no_copy=no_copy,err=err
...
function linked_list::have_item,index,err=err
...
FUNCTION LINKED_LIST::GET_ITEM, index,err=err
...
FUNCTION LINKED_LIST::GET_NODE, index,err=err
...
PRO LINKED_LIST::HELP, Print=print,err=err
...
dprint,'% LINKED_LIST::CLEANUP'

self->Delete_Nodes
if ptr_valid(self.head) then heap_free, self.head
if ptr_valid(self.tail) then heap_free, self.tail
END
...
; The implementation of a LINKED_LIST object.

   struct = { LINKED_LIST, $         ; The LINKED_LIST object.
              head:Ptr_New(), $     ; A pointer to the first node.
              tail:Ptr_New(), $     ; A pointer to the last node.
              count:0L $            ; The number of nodes in the list.
              }

   struct = { LINKED_LIST_NODE, $    ; The LINKED_LIST NODE structure.
