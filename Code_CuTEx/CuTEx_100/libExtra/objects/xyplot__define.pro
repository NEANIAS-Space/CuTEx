...
pro xyplot::empty,deallocate=deallocate

self->free_var,no_deallocate=1-keyword_set(deallocate)
...
if not sum then return,0b

xtype=self->get(/xtype)
if (xtype eq 2) or (xtype eq 3) then begin
 message,'Y-DATA summing is disabled for XTYPE = '+strtrim(xtype,2),/cont
...
if is_string(extra) and not keyword_set( no_set_plot ) then begin
...
if (not self->has_xdata()) or (not self->has_ydata()) then return,0b
...
pro xyplot::set_data,xdata,ydata,edata,id=id,dim1_ids=dim1_ids,dim1_vals=dim1_vals,$
           data_unit=data_unit,dim1_unit=dim1_unit,plot_type=plot_type,$
...
if not keyword_set( no_defaults ) then begin
    self->set_channels
    self->set_linestyles
    self->set_colors
    self->set_chan_ids
...
ny=self->get(/ny)
if not exist(dim1_ids) then dim1_ids=strarr(ny)
...
pro xyplot::set_plot,ylog=ylog,all=all,dim1_enab_sum=dim1_enab_sum,$
             ...
             charsize=charsize,dim1_use=dim1_use,xlog=xlog,histogram=histogram, psym=psym
...
if not exist(yrange) then self.yrange=[0.,0.] else $
 if valid_range(yrange,/allow_zeros) then self.yrange=float(yrange)
...
if not exist(xrange) then self.xrange=[0.d,0.d] else $
...
if not self->has_xdata() then begin
...
if not self->has_ydata() then begin
...
; title added by acs 2004-07-21 for use w/ spectrograms
                     title = title
...
if not self->check_data(err_msg=err_msg) then return

cancel=0b
if not self->has_ydata() then self->options,cancel=cancel,title='Filename: '+self->get(/filename)
...
if (not valid_range(yrange)) then begin
...
;-- plot axis first without data
  ...
  if (pcount eq 1) and (not ioverlay) then begin
...
wshow2
if (pcount eq 0) and (not ioverlay) then $
...
if obj_isa(self.overlay_obj, 'xyplot') then begin
	if not use_colors then begin
		save_colors = self.overlay_obj->get(/dim1_colors)
		temp_colors = ' '
	endif
	; share the following params with the overlay plot, kim 4-apr-07
	o_extra = struct_subset(extra, ['charsize','thick', 'psym', 'nsum'], status=status)
	if not status then o_extra={dummy:0}
	self.overlay_obj->plot,/overlay, /no_timestamp, dim1_colors=temp_colors, _extra=o_extra
	if not use_colors then self.overlay_obj -> set, dim1_colors=save_colors
endif
...
if not exist(edata) then return
...
if not ptr_valid(self.ydata_ptr) then return,0b
...
if not ptr_valid(self.edata_ptr) then return,0b
...
if not ptr_valid(self.xdata_ptr) then return,0b
...
if not exist(channels) then valid=0 else valid=channels
ny=self->get(/ny)
if ny gt 0 then begin
 ok=where( (valid lt ny) and (valid gt -1),count)
 if count gt 0 then valid=valid[ok] else valid=0
...
ny=self->get(/ny)
if not exist(dim1_colors) or is_string(dim1_colors, /blank) then begin
...
if !p.background eq black then begin
 ...
 if count gt 0 then cc[iwhite]=white
endelse
...
if not exist(dim1_linestyles) then dim1_linestyles=indgen(6)
...
label_size = ch_scale(1., /xy)
time_size = ch_scale(.6, /xy)
if exist(charsize) then begin
 label_size = ch_scale(charsize, /xy)
...
if loc ne 0 then begin
 ...
 cmd = 'legend, text, box=0, charthick=charthick'
...
;-- linestyle -99 means don't draw line or indent
 ...
 if not (n_elements(text) eq 1 and text[0] eq '') then begin
...
if n_elements(dim1_vals) eq 2 or size(dim1_vals, /n_dimen) eq 2 then begin
 ...
 if ranges[0] ne -1 then begin
  if self.dim1_is_time then begin   ; dim1 is times
   text2 = reform(anytim(ranges[0,*],/vms) + ' - ' $
           + anytim(ranges[1,*],/vms,/time))
  endif else begin
   sranges = strtrim (string (ranges, format='(f9.1)'), 2)
   sranges = reform(sranges, size(ranges, /dim))
   r_string = sranges[0,*] + '  -  ' + sranges[1,*]
   text2 = reform(r_string) + ' ' + self->get(/dim1_unit)
  endelse
 endif
endif
return, text2
...
count=0
if not valid_range(range) then return,-1
...
if not weighted_sum then begin
...
sum=self->allow_sum()
if not sum then begin
 if not self->valid_chan(chan,err_msg=err_msg) then return,-1
endif

if sum then ydata=self->get_sum_ydata() else $
 ydata=reform((*self.ydata_ptr)[*,chan])

if (self->has_edata()) and (not sum) then begin
...
function xyplot::get_xdata,chan,err_msg=err_msg,midpoint=midpoint
...
case xtype of
 ...
 1: if midpoint then return,total(*self.xdata_ptr,1)/2. else return,*self.xdata_ptr
 2: return,reform((*self.xdata_ptr)[*,chan])
 3: if midpoint then return,reform(total((*self.xdata_ptr)[*,*,chan],1)/2.) else $
...
err_msg=''
if not is_number(chan) then chan=0
ny=self->get(/ny)
