...
pro ftp::fget,file,out_dir=out_dir,copy_file=copy_file,_ref_extra=extra,$
         rsize=rsize,err=err,need_size=need_size,status=status,$
         nowait=nowait,limit=limit,nbackground=nbackground
...
if nowait then begin
 if is_number(nbackground) then begin
  if (1-is_number(limit)) then limit=3
...
if arg_present(rsize) or need_size then begin
 read_ftp,rhost[0],nfile,rsize=rsize
...
self->setprop,rhost=rhost[0],rfile=rfile,lfile=rfile,rdir=rdir,ldir=out_dir,$
              _extra=extra,bytes=0,clobber=0
