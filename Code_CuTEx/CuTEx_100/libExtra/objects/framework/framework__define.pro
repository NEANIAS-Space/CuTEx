...
; Time-stamp: <Wed May 16 2007 12:22:04 csillag soleil.i4ds.ch>
;---------------------------------------------------------------------------
;+
; PROJECT:
;       HESSI
...
;help, n_elements( found ), this_struct, /str
...
IF Keyword_Set( CLASS_NAME ) THEN BEGIN
    ...
    IF Obj_Valid( this_object ) THEN BEGIN
        RETURN, this_object->GetData( DIMENSION=dimension, $
                                      ...
                                      _EXTRA=_extra )
    ENDIF ELSE BEGIN
        Message, 'This class has not yet been instanciated.';, /CONTINUE
        RETURN, -1
    ENDELSE
ENDIF
...
IF (self->Need_Update() OR force) AND NOT Keyword_Set( NOPROCESS ) THEN BEGIN
...
;        IF Obj_Class( self ) EQ 'HSI_EVENTLIST' then stop
;        IF Obj_Valid( this_object ) THEN BEGIN
            this_object->Set, _EXTRA = _extra, DONE=this_done, $
              NOT_found=this_NOT_found, $
