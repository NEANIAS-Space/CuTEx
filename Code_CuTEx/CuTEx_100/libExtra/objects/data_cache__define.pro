...
common data_cache,fifo
if not obj_valid(fifo) then return,0b
fifo->get,id,data,status=status
return,status                               
...
common data_cache,fifo
if not obj_valid(fifo) then return

id=self.name                               

fifo->get,id,data,status=status

if status and self.verbose then message,'restoring from "cache_'+id+'"',/cont
return & end
