...
goesdig=is_member(data_type,['G61','G71','G65','G75','G81','G85','G91','G95',$
				   'G01','G05','G11','G15','G21','G25','G35'],/ignore_case)
...
while (not qdone) do begin
	...
	case strupcase(data_type) of
	    ...
	    'NEL': begin
			...
			infil = weekid(wid, prefix=strlowcase(data_type),indir=indir, vnum=vnum,verbose=verbose,remote=remote)
		  end
	endcase
	...
	if (infil eq '') then begin
	    ...
	    message, 'No input file for week ' + wid + ' found in: ' + indir, /info
            ...
            chk=loc_file(infil,count=count,/recheck)
            if (count eq 0) then begin
             ...
             if network then begin
              ...
              sfile=infil
              if keyword_set(verbose) then message,'Downloading '+sfile,/cont
              sock_copy,sfile,out=temp_dir,verbose=verbose
             endif
	    ...
	    openr, lun, infil, /get_lun, /block
            if keyword_set(verbose) then message,'Reading Yohkoh weekly file '+infil,/cont
	    ;
...
if is_dir(comdir) and (1-remote) then gen=loc_file(gfiles,count=count,/recheck)
if (count eq 0) or remote then begin
 if keyword_set(verbose) then message,'Checking remote GOES server...',/cont
 server=goes_server(network=network)
 break_file,gfiles,dsk,dir,name,ext
 if network then begin
  path=concat_dir(ydb_path(),strlowcase(preweek))
  gen=sock_find(server,name+ext,path=path)
  gen=local_name(gen)
 endif else gen=loc_file(name+ext,path=goes_temp_dir())
endif
