	...
	'plot': begin
		...
		if state.use_plotman then begin
			...
			if num_plot gt 1 then begin
				; only allow 3 overlays and primary
				if num_plot gt 4 then message,'Warning, Max number of plots in multi-plot is 4 for PLOTMAN.', /cont
				ov = ov[0:(num_plot-2)<2]
				overlay_panel = strarr(4)
				overlay_panel[1] = ov	;0th is reserved for self (for images) - use 1,2,3
