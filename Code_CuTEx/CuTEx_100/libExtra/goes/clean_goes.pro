...
; numstat not equal to -1 means the data file contained status information
; Use that to find gain changes.  Otherwise do it manually.
if numstat ne -1 then find_fits_bad, tarray, yarray, bad0, bad1, numstat, $
      tstat, stat
...
;
   ...
   if numstat ne -1 then if ich eq 0 then bad = bad0 else bad = bad1 else begin
	...
	(trel(1:*)-trel)/3.0 ) )* test ge 1 and y gt ymin, nbad)
...
;
	if n_elements( wbad) ge 50 then begin
		...
		if nshort ge 1 then begin
			wbad2 = 0
			for i=0,nshort-1 do wbad2 = [wbad2, length(wshort(i))+wcont(0,i)]
			wbad = wbad(wbad2(1:*))
		endif else begin
			wbad = -1
		endelse
