...
; - add the calibration data to the goes_tf_coeff, goes_tf routines
...
gbshort = 1e-5*[1.27,1.25,1.25,1.73,1.74,1.74,1.68,1.580,1.607,1.631,1.608,1.595] 
gblonga = 1e-6*[4.09,3.98,3.98,4.56,4.84,5.32,4.48,4.165,3.990,3.824,4.377,4.090] 
gblongb = gblonga & gblongb(5)=4.43e-6      ; Before & after for GOES-6 
 
;  Transfer function coefficients for all GOES satellites: 
coeff=dblarr(12,5,7) & wsm=dblarr(55,12) & wlm=wsm & tsm=wsm & tlm=wsm 
...
if satnum lt 1 or satnum gt 12 then begin 
  print,'Only satellite numbers 1 - 12 are presently supported!' 
  goto,exit 
  endif 
...
printf,1,';'
printf,1,'; Usage:'
printf,1,';     goes_get_chianti_temp, ratio, temperature, sat=goes, /photospheric'
printf,1,';'
printf,1,';Purpose:'
...
printf,1,''
printf,1,'r_cor=fltarr('+strtrim(string(nsat),2)+',101)     ; ratio vs temp for each of 12 GOES satellites'
printf,1,''
...
printf,2,''
printf,2,'b8_cor=fltarr('+strtrim(string(nsat),2)+',101)     ; responses for 101 temps for each of 12 GOES satellites'
printf,2,''
...
printf,1,''
printf,1,'if keyword_set(sat) then gsat=fix(sat-1)>0 else gsat=8-1 ; subtract 1 to get array index'
...
printf,2,''
printf,2,'if keyword_set(sat) then gsat=fix(sat-1)>0 else gsat=8-1 ; subtract 1 to get array index'
...
pro make_goes_chianti_response
 ...
 nsat=12             ; current number of satellites in goes_tf_coeff
 fold_spec_resp, nsat
