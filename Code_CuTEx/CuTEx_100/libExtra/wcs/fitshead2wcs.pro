...
w = where((strmid(ctype,1,4) eq 'LON-') or (strmid(ctype,2,3) eq 'LN-') or $
          (strmid(ctype,0,4) eq 'HECX') or (ctype eq 'SOLARX') or $
          (ctype eq 'SOLAR-X') or (ctype eq 'SOLX'), count)
if count gt 0 then ix = w[0] else begin
    compliant = 0
    ix = 0
endelse
w = where((strmid(ctype,1,4) eq 'LAT-') or (strmid(ctype,2,3) eq 'LT-') or $
          (strmid(ctype,0,4) eq 'HECY') or (ctype eq 'SOLARY') or $
          (ctype eq 'SOLAR-Y') or (ctype eq 'SOLY'), count)
...
for i=1,n_axis do begin
;
;  If CRPIX is not found, then use the center of the array.
;
    val = wcs_find_keyword(index, tags, column, system, count, $
      'CRPIX' + ntrim(i), ntrim(i) + ['CRPX','CRP'], lunfxb=lunfxb, $
      rowfxb=rowfxb)
    if count gt 0 then crpix[i-1] = val[0] else begin
        compliant = 0
        crpix[i-1] = (naxis[i-1] + 1.d0) / 2.d0
    endelse
;
;  If CRVAL is not found, use XCEN,YCEN for the longitude and latitude
;  dimensions.  Otherwise, the default is 0.
;
    val = wcs_find_keyword(index, tags, column, system, count, $
    ...
    if count gt 0 then crval[i-1] = val[0] else begin
        compliant = 0
        if i eq ix then crval[i-1] = xcen
        if i eq iy then crval[i-1] = ycen
    endelse
;
...
;
            ...
            if count gt 0 then cdelt[i-1] = val[0] else begin
                compliant = 0
                if i eq ix then cdelt[i-1] = dx
                if i eq iy then cdelt[i-1] = dy
            endelse
;
...
;
;  If not determined yet, and both the X and Y axes are angular, then assume
;  the TAN projection.
;
if projection eq '' then begin
...
wcs_find_time,     index, tags, system, wcs, column=column, $
  lunfxb=lunfxb, rowfxb=rowfxb
wcs_find_position, index, tags, system, wcs
