...
function make_mask_new, ima, region, header=header, nan=nan

ch = size(ima)

if ch(0) eq 0 then im = readfits(ima,header,/silent) else begin
...
if keyword_set(nan) then begin
	rr = where(mask eq 0.,nn)
	mask[rr] = !values.f_nan
endif

im_mask = im*mask

if ch(0) eq 0 then writefits, strmid(ima, 0 , strpos(ima, '.fits'))+'_mask.fits', im_mask, header else writefits, outdir+'masked.fits', im_mask, header
